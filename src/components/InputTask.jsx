...
const InputTask = ({ label, type, className, ...props },ref) => {
  const id = useId();
  return (
    <div>
      <div className="relative w-full min-w-[100px] h-10 m-1">
        <input
          className={`peer w-full h-full text-gray-700 font-sans font-normal outline outline-0 focus:outline-0 disabled:bg-gray-400 disabled:border-0 transition-all placeholder-shown:border placeholder-shown:border-gray-400 placeholder-shown:border-t-gray-400 border focus:border-2 border-t-transparent focus:border-t-transparent text-sm px-3 py-2.5 rounded-[7px] border-gray-400 focus:border-gray-900 ${className}`}
          placeholder=" "
          type={type}
          id={id}
          key={label}
...
export default React.forwardRef(InputTask);
